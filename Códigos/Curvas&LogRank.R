# Universidade Federal Fluminense
# Instituto de matemática e estatística
# Departamento de estatística
# Iniciação científica
# Orientadora: Núbia Karla de Oliveira Almeida
# Bolsista FAPERJ: Pedro Frazão Dutra

## Carregamento dos pacotes necessários
library(tidyverse)
library(survival)
library(survminer)
library(gridExtra)

## Carregamentos dos dados
inca <- read_csv2("inca.csv")

inca2 <- inca |>
  filter(
    COMPBIO %in% c("2", "3"),
    ESTCONJ %in% c("1", "2", "3", "4", "5"),
    INSTRUC %in% c("1", "2", "3", "4", "5", "6"),
    RACACOR %in% c("1", "2", "3", "4"),
    !ESTADIAM %in% c("99", "B2", "88"),
    !LATERALI %in% c("0","8","9"),
    !MAISUMTU %in% c("0","3"),
    !PRITRATH %in% c("9"),
    as.numeric(IDADE) >= 20,
    HISTFAMC %in% c("1","2")
  ) |>
  mutate(IDADE = as.numeric(IDADE))

inca3 <- inca2 |>
  mutate(
    
    atraso_pricon_diag = as.numeric(DtDiagno - DataPricon),
    hiato2 = case_when(
      atraso_pricon_diag <= 30 ~ "(0,30]",
      atraso_pricon_diag > 30 & atraso_pricon_diag <= 60 ~ "(30,60]",
      atraso_pricon_diag > 60 ~ "60+"
    ),
    
    faixaEtaria = case_when(
      IDADE < 40 ~ "[20,40)",
      IDADE >= 40 & IDADE < 60 ~ "[40,60)",
      IDADE >= 60 ~ "60+"
    ),
    tempo = sobrevida / 30.4375,
    companheiro = case_when(
      ESTCONJ %in% c("1","3","4") ~ "Sem companheiro",
      ESTCONJ %in% c("2","5") ~ "Com companheiro"
    ),
    ESCOL = case_when(
      INSTRUC == 1 ~ 1,
      INSTRUC == 2 ~ 2,
      INSTRUC %in% c(3, 4) ~ 3,
      INSTRUC %in% c(5, 6) ~ 4
    ),
    estadiamento = case_when(
      ESTADIAM == "0" ~ 0,
      ESTADIAM %in% c("1","1A","1B") ~ 1,
      ESTADIAM %in% c("2","2A","2B") ~ 2,
      ESTADIAM %in% c("3","3A","3B","3C") ~ 3,
      ESTADIAM %in% c("4","4A","4B") ~ 4
    ),
    historico = case_when(
      HISTFAMC == "1" ~ "Sim",
      HISTFAMC == "2" ~ "Não"
    ),
    hiato = case_when(
      atraso <= 30 ~ "(0,30]",
      atraso > 30 & atraso <= 60 ~ "(30,60]",
      atraso > 60 ~ "60+"
    )
  )

# Criando as legendas
inca4 <- inca3 |> 
  mutate(
    escolaridade = case_when(
      ESCOL == "1" ~ "Nenhum",
      ESCOL == "2" ~ "Fund.Incompleto",
      ESCOL == "3" ~ "Fund. / Médio",
      ESCOL == "4" ~ "Sup. / Sup.Incompleto"
    ),
    racaCor = case_when(
      RACACOR == "1" ~ "Branca",
      RACACOR == "2" ~ "Preta",
      RACACOR == "3" ~ "Amarela",
      RACACOR == "4" ~ "Parda",
      RACACOR == "5" ~ "Indígena"
    ),
    estadoConjugal = case_when(
      ESTCONJ == "1" ~ "Solteiro",
      ESTCONJ == "2" ~ "Casado",
      ESTCONJ == "3" ~ "Viúvo",
      ESTCONJ == "4" ~ "Separado judicialmente",
      ESTCONJ == "5" ~ "União consensual"
    ),
    lateralidade = case_when(
      LATERALI == "1" ~ "Direita",
      LATERALI == "2" ~ "Esquerda",
      LATERALI == "3" ~ "Bilateral"
    ),
    maisumtu = case_when(
      MAISUMTU == "1" ~ "Não",
      MAISUMTU == "2" ~ "Sim"
    )
  ) 

inca4$escolaridade <- as.factor(inca4$escolaridade)

inca4$escolaridade <- factor(
  inca4$escolaridade,
  levels = c(
    "Nenhum",
    "Fund.Incompleto",
    "Fund. / Médio",
    "Sup. / Sup.Incompleto"
  ),
  ordered = TRUE
)

inca4 <- inca4 |> 
  filter(racaCor != "Amarela",
         racaCor != "Indígena")

inca5 <- inca4 |>
  dplyr::mutate(
    fezCirurgia = ifelse(
      grepl("2", PRITRATH),
      "Sim",
      "Não"
    )
  )

inca5 <- inca5 |>
  dplyr::mutate(
    fezRadio = ifelse(
      grepl("3", PRITRATH),
      "Sim",
      "Não"
    ),
    fezQuimio = ifelse(
      grepl("4", PRITRATH),
      "Sim",
      "Não"
    )
  )

inca6 <- inca5 |> 
  select(status,tempo,companheiro,escolaridade,racaCor,faixaEtaria,
         estadiamento,lateralidade,maisumtu,fezCirurgia,fezRadio,fezQuimio,
         historico,hiato,hiato2,ESTDFIMT,IDADE) |> 
  mutate(
    escolaridade = as.factor(escolaridade),
    racaCor = as.factor(racaCor),
    faixaEtaria = as.factor(faixaEtaria),
    companheiro = as.factor(companheiro),
    estadiamento = as.factor(estadiamento),
    lateralidade = as.factor(lateralidade),
    maisumtu = as.factor(maisumtu),
    fezQuimio = as.factor(fezQuimio),
    fezRadio = as.factor(fezRadio),
    fezCirurgia = as.factor(fezCirurgia),
    atrasoTrat = as.factor(hiato),
    atrasoDiag = as.factor(hiato2))|> filter(is.na(estadiamento) == F,
                                             is.na(atrasoTrat) == F,
                                             is.na(atrasoDiag) == F) 

inca6$escolaridade <- factor(inca6$escolaridade, levels = c(
  "Nenhum", 
  "Fund.Incompleto", 
  "Fund. / Médio", 
  "Sup. / Sup.Incompleto"
))

# write.csv2(inca6,"baseDashboard.csv", row.names = F)

y <- Surv(inca6$tempo / 12, inca6$status)

km <- survfit(y ~ 1, data = inca6)

graf <- ggsurvplot(km, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = " ",
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23, "black"),
                    font.y = c(23, "black"),
                    lwd = 1.2,
                   legend = "none")

graf <- graf$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.94, 1)) +
  theme(
    plot.title = element_text(hjust = 0.5),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf

#################################
# Gráfico segundo estadiamento
km1 <- survfit(y~inca6$estadiamento, data = inca6)

graf1 <- ggsurvplot(km1, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Estadiamento",
                    legend.labs = c("0", "1","2","3","4"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23, "black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf1 <- graf1$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.05),
                     limits = c(0.75, 1)) +
  scale_color_manual(values = c(
    "0" = "#fedc97",
    "1" = "#b5b682",
    "2" = "#7c9885",
    "3" = "#28666e",
    "4" = "#033f63"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 20),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
    )

graf1

logrank1 <- survdiff(y~inca6$estadiamento, data = inca6)
logrank1[["pvalue"]]

#################################

# Gráfico segundo a ocorrência de mais um tumor
km2 <- survfit(y~inca6$maisumtu, data = inca6)

graf2 <- ggsurvplot(km2, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Múltiplos tumores",
                    legend.labs = c("Não","Sim"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23,"black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf2 <- graf2$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Não" = "#0d1b2a",
    "Sim" = "#778da9"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5,size = 20),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
    )

graf2

logrank2 <- survdiff(y~inca6$maisumtu, data = inca6)
logrank2[["pvalue"]]

#################################

# Gráfico segundo a lateralidade do tumor
km3 <- survfit(y~inca6$lateralidade, data = inca6)

graf3 <- ggsurvplot(km3, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Lateralidade",
                    legend.labs = c("Bilateral","Direita","Esquerda"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23, "black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf3 <- graf3$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.88, 1)) +
  scale_color_manual(values = c(
    "Bilateral" = "#EF476F",
    "Direita" = "#FFD166",
    "Esquerda" = "#06D6A0"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black"))

graf3

logrank3 <- survdiff(y~inca6$lateralidade, data = inca6)
logrank3[["pvalue"]]

################################# 
# Gráfico segundo faixa etária
km4 <- survfit(y~inca6$faixaEtaria, data = inca6)

graf4 <- ggsurvplot(km4, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Faixa Etária",
                    legend.labs = c("[20,40)", "[40,60)","60+"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23, "black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf4 <- graf4$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "[20,40)" = "#EF476F",
    "[40,60)" = "#FFD166",
    "60+" = "#06D6A0"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 20),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf4

logrank4 <- survdiff(y~inca6$faixaEtaria, data = inca6)
logrank4[["pvalue"]]

#################################

# Gráfico segundo a escolaridade
km5 <- survfit(y ~ escolaridade, data = inca6)

graf5 <- ggsurvplot(
  km5,
  censor = FALSE,
  xlab = "Anos",
  ylab = expression(hat(S)(t)),
  ggtheme = theme_minimal(),
  title = "Escolaridade",
  legend.labs = levels(inca6$escolaridade),
  font.tickslab = c(11, "plain", "black"),
  font.x = c(23, "black"),
  font.y = c(23, "black"),
  lwd = 1.2
)

graf5 <- graf5$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02), limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Nenhum" = "#fedc97",
    "Fund.Incompleto" = "#b5b682",
    "Fund. / Médio" = "#7c9885",
    "Sup. / Sup.Incompleto" = "#28666e"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )
graf5

logrank5 <- survdiff(y~inca6$escolaridade, data = inca6)
logrank5[["pvalue"]]

#################################
# Gráfico segundo a raça/cor
km6 <- survfit(y~inca6$racaCor, data = inca6)

graf6 <- ggsurvplot(km6, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Raça/Cor ",
                    legend.labs = c("Branca","Parda","Preta"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23, "black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf6 <- graf6$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Branca" = "#c1c1c1",
    "Parda" = "#d16666",
    "Preta" = "#2c4251"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf6

logrank6 <- survdiff(y~inca6$racaCor, data = inca6)
logrank6[["pvalue"]]

#################################
# Gráfico segundo o relacionamento romântico
km7 <- survfit(y~inca6$companheiro, data = inca6)

graf7 <- ggsurvplot(km7, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Estado civil",
                    legend.labs = c("Com companheiro","Sem companheiro"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23, "black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf7 <- graf7$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Com companheiro" = "#fac05e",
    "Sem companheiro" = "#ee6352"
  )) +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf7

logrank7 <- survdiff(y~inca6$companheiro, data = inca6)
logrank7[["pvalue"]]

#################################

# Sobrevivência segundo a realização de cirurgia
km8 <- survfit(y~inca6$fezCirurgia, data = inca6)

graf8 <- ggsurvplot(km8, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Cirurgia",
                    legend.labs = c("Não","Sim"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23,"black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf8 <- graf8$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.84, 1)) +
  scale_color_manual(values = c(
    "Não" = "#0d1b2a",
    "Sim" = "#778da9"
  ))  +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
    )

graf8

logrank8 <- survdiff(y~inca6$fezCirurgia, data = inca6)
logrank8[["pvalue"]]

# Sobrevivência segundo a realização de quimioterapia
km9 <- survfit(y~inca6$fezQuimio, data = inca6)

graf9 <- ggsurvplot(km9, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Quimioterapia",
                    legend.labs = c("Não","Sim"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23,"black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf9 <- graf9$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Não" = "#0d1b2a",
    "Sim" = "#778da9"
  ))  +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf9

logrank9 <- survdiff(y~inca6$fezQuimio, data = inca6)
logrank9[["pvalue"]]

# Sobrevivência segundo a realização de radioterapia
km10 <- survfit(y~inca6$fezRadio, data = inca6)

graf10 <- ggsurvplot(km10, censor = FALSE,
                    xlab = "Anos", ylab = expression(hat(S)(t)),
                    ggtheme = theme_minimal(),
                    title = "Radioterapia",
                    legend.labs = c("Não","Sim"),
                    font.tickslab = c(11, "plain", "black"),
                    font.x = c(23,"black"),
                    font.y = c(23, "black"),
                    lwd = 1.2)

graf10 <- graf10$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Não" = "#0d1b2a",
    "Sim" = "#778da9"
  ))  +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf10

logrank10 <- survdiff(y~inca6$fezRadio, data = inca6)
logrank10[["pvalue"]]

# Sobrevivência segundo histórico familiar de câncer
km11 <- survfit(y~inca6$historico, data = inca6)

graf11 <- ggsurvplot(km11, censor = FALSE,
                     xlab = "Anos", ylab = expression(hat(S)(t)),
                     ggtheme = theme_minimal(),
                     title = "Histórico familiar de câncer",
                     legend.labs = c("Não","Sim"),
                     font.tickslab = c(11, "plain", "black"),
                     font.x = c(23,"black"),
                     font.y = c(23, "black"),
                     lwd = 1.2)

graf11 <- graf11$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "Não" = "#0d1b2a",
    "Sim" = "#778da9"
  ))  +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf11

logrank11 <- survdiff(y~inca6$historico, data = inca6)
logrank11[["pvalue"]]

# Sobrevivência segundo atraso até início do tramento
km12 <- survfit(y~inca6$hiato, data = inca6)

graf12 <- ggsurvplot(km12, censor = FALSE,
                     xlab = "Anos", ylab = expression(hat(S)(t)),
                     ggtheme = theme_minimal(),
                     title = "Tempo decorrido até o tratamento",
                     legend.labs = c("(0,30]","(30,60]","60+"),
                     font.tickslab = c(11, "plain", "black"),
                     font.x = c(23,"black"),
                     font.y = c(23, "black"),
                     lwd = 1.2)

graf12 <- graf12$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "(0,30]" = "#c1c1c1",
    "(30,60]" = "#d16666",
    "60+" = "#2c4251"
  ))  +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf12

logrank12 <- survdiff(y~inca6$hiato, data = inca6)
logrank12[["pvalue"]]

# Sobrevivência segundo atraso até o diagnóstico
km13 <- survfit(y~inca6$hiato2, data = inca6)

graf13 <- ggsurvplot(km13, censor = FALSE,
                     xlab = "Anos", ylab = expression(hat(S)(t)),
                     ggtheme = theme_minimal(),
                     title = "Tempo decorrido até o diagnóstico",
                     legend.labs = c("(0,30]","(30,60]","60+"),
                     font.tickslab = c(11, "plain", "black"),
                     font.x = c(23,"black"),
                     font.y = c(23, "black"),
                     lwd = 1.2)

graf13 <- graf13$plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.02),
                     limits = c(0.92, 1)) +
  scale_color_manual(values = c(
    "(0,30]" = "#c1c1c1",
    "(30,60]" = "#d16666",
    "60+" = "#2c4251"
  ))  +
  theme(
    legend.position = "right",
    legend.text = element_text(size = 23, color = "black"),
    legend.title = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 23),
    axis.text.x = element_text(size = 23, color = "black"),
    axis.text.y = element_text(size = 23, color = "black")
  )

graf13

logrank13 <- survdiff(y~inca6$hiato2, data = inca6)
logrank13[["pvalue"]]

# Terminação
