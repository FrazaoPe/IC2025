...
dados19C <- dados19 %>%
  mutate(
    DATAINITRT = ifelse(DATAINITRT == "6 /05/2019", "06/05/2019", DATAINITRT),
    DATAINITRT = ifelse(DATAINITRT == "9 /07/2019", "09/07/2019", DATAINITRT)
)
...
dep19F <- dados19C |> filter(SEXO == 2,
                          LOCTUDET == "C50")

write.csv2(dep19F, "rhc19F.csv", row.names = FALSE)
