...
dados16C <- dados16 %>%
  mutate(
    DATAINITRT = ifelse(DATAINITRT == "30/7 /2016", "30/07/2016", DATAINITRT),
    DTTRIAGE = ifelse(DTTRIAGE == "5 /08/2016","05/08/2016",DTTRIAGE),
  )
...
dep16F <- dados16C |> filter(SEXO == 2,
                          LOCTUDET == "C50")

write.csv2(dep16F, "rhc16F.csv", row.names = FALSE)
