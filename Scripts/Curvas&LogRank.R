...
write.csv2(inca6,"baseDashboard.csv",row.names = F,col.names = F)
...
inca5$historico <- as.factor(inca5$historico)

levels(inca5$historico)

km11 <- survfit(y~inca5$historico, data = inca5)
...
logrank11 <- survdiff(y~inca5$historico, data = inca5)
logrank11[["pvalue"]]
# p-valor < 0.01

# Sobrevivência segundo atraso até início do tramento
inca5$hiato <- as.factor(inca5$hiato)

levels(inca5$hiato)

km12 <- survfit(y~inca5$hiato, data = inca5)

graf12 <- ggsurvplot(km12, censor = FALSE,
                     xlab = "Anos", ylab = expression(hat(S)(t)),
                     ggtheme = theme_minimal(),
                     title = "Atraso para o tratamento",
...
logrank12 <- survdiff(y~inca5$hiato, data = inca5)
logrank12[["pvalue"]]
# p-valor < 0.01
